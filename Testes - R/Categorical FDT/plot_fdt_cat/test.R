...
# Dados de teste
df <- data.frame(
  Category = c("Segurança", "Trânsito", "Trans. Público", "Saúde", "Educação", "Outros"),
  Frequency = c(18, 17, 16, 7, 5, 3),
  `Relative Frequency` = c(0.27, 0.26, 0.24, 0.11, 0.08, 0.05),
  `Relative Frequency %` = c(27, 26, 24, 11, 8, 5),
  `Cumulative Frequency` = c(18, 35, 51, 58, 63, 66),
  `Cumulative Frequency %` = c(27, 53, 77, 88, 95, 100)
)

# Tipos de gráficos para teste
types <- c('fb', 'fp', 'fd', 'rfb', 'rfp', 'rfd', 'rfpb', 'rfpp', 'rfpd', 
           'cfb', 'cfp', 'cfd', 'cfpb', 'cfpp', 'cfpd', 'pa')

# Loop para gerar e salvar cada gráfico
for (t in types) {
  plot.fdt_cat.default(df, type = t, xlab = "Category", ylab = "Values", main = paste("Type:", t))
}

cat("Gráficos em R gerados com sucesso.\n")